...
class TestAbstractMessageBus:
    ...
    def test_command_handling_workflow(self, message_bus, uow, create_user_command, user_created_event):
        ...
        message_bus._collect_new_events.assert_has_calls([
            call(uow),
            call(uow)
        ])
    ...
    def test_event_chain_processing(self, message_bus, uow, user_created_event):
        """이벤트 체인 처리를 테스트합니다."""
        # Arrange
        notification_event = FakeUserCreatedEvent("123", "testuser")  # 실제로는 다른 이벤트 타입을 사용해야 함
        ...
        message_bus._handle_event.assert_has_calls([
            call(user_created_event, uow),
            call(notification_event, uow)
        ])
        assert message_bus._collect_new_events.call_count == 2
