...
class TestContainerConfiguration:
    ...
    def test_container_has_required_providers(self):
        ...
        for provider in essential_providers:
            assert hasattr(container, provider), f"Container should have {provider} provider"
    ...
    def test_config_loading(self, test_config):
        ...
        assert container.config.persistence.config.connect_args.check_same_thread() is False

        # security 설정 검증
        assert container.config.security.jwt_generator.type() == "jwt"
        assert container.config.security.jwt_generator.config.jwt_secret() == "test-secret"
        ...
        assert container.config.security.password_hasher.config.password_work_factor() == 4
