...
class Project:
    ...
    def invite_member(
        self, email: str, role: ProjectRole
    ) -> tuple[Self, "ProjectInvitation"]:
        """멤버 초대
    ...
    def accept_invitation(self, code: str, user: User) -> tuple[Self, "ProjectMember"]:
        ...
        if invitation.email != user.email:
            raise InvalidInvitationCodeError(
                f"Invitation was sent to {invitation.email}, not {user.email}"
            )
        ...
        return (
            replace(
                self, members=[*self.members, new_member], invitations=new_invitations
            ),
            new_member,
        )
